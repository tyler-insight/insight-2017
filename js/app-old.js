    ...
    function safariResize() {
      ...
      $('img').on('load',function(){
        ...
        if (loaded == $('img').length){
          for (var i = 1; i < images.length; i++){
            ...
            } else if (($(images[i]).css("object-fit") == 'cover') && ($(images[i]).parent().hasClass("project-image-wrapper"))){
              var parentH = $(images[i]).parent().height();
              var parentW = $(images[i]).parent().width();
              var newNewHRatio = parentH / newH;
              var newNewWRatio = parentW / newW;
              newNewH = parentH * newNewWRatio;
              newNewW = parentW * newNewHRatio;

              var newLeft = (newNewW - parentW) / 2;
                $(images[i]).css("height", parentH + "px");
                $(images[i]).css("width", newNewW + "px");
                $(images[i]).css("max-height", "2000px");
                $(images[i]).css("max-width", "2000px");
                $(images[i]).css("position", "relative");
                 $(images[i]).css("left", "-" + newLeft + "px");

            } else if (($(images[i]).css("object-fit") == 'cover') && (!$(images[i]).parent().hasClass("project-image-wrapper"))){
              newNewW = $(images[i]).width();
              $(images[i]).css("width", "100%");
              newNewH = $(images[i]).width() * ratio;
                $(images[i]).css("height", newNewH + "px");
      ...
      $( window ).resize(function() {
        ...
        for (var i = 1; i < images.length; i++){
          ...
          var naturalW = images[i].naturalWidth;
          var ratio = naturalH / naturalW;
          var newRatio = ($(images[i]).parent().width() / naturalW);
          var newH = naturalH * newRatio;
          var newW = naturalW * newRatio;

          if ($(images[i]).css("object-fit") != 'cover'){
            if (!$(images[i]).parent().hasClass(employee)) {
              $(images[i]).css("height", newH + "px");
            }
            ...
            var newNewHRatio = parentH / newH;
            var newNewWRatio = parentW / newW;
            var newNewH = parentH / newNewHRatio;
            var newNewW = parentW * newNewHRatio;
            if (newNewW < parentW){
              fixedRatio = parentW / newNewW;
              newNewW = newNewW * fixedRatio;
            }
              ...
              if (newNewW > parentW){
                ...
                newLeft = (parentW - newNewW) / 2;
              } else if (newNewW < parentW){
                newLeft = (newNewW - parentW) / 2;
              } else {
                newLeft = 0;
              }
             $(images[i]).css("left",  newLeft + "px");
